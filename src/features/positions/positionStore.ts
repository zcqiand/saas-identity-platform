...
interface State { list: Position[]; loading: boolean; error: string | null }
interface Actions {
...
function extract(err: unknown) {
  return ((err as { response?: { data?: { message?: string } }; message?: string }).response?.data?.message ?? (err as Error).message ?? '操作失败')
}
...
export const usePositionStore = create<Store>()((set, get) => ({
  ...
  fetchPositions: async () => {
    ...
    try { set({ list: (await apiClient.get<Position[]>('/positions')).data, loading: false }) }
    catch (e) { set({ loading: false, error: extract(e) }) }
  },
