...
export default function OpenPlatformPage() {
  ...
  const update = async (patch: Partial<OpenPlatformConfig>) => {
    setSaving(true)
    try { const { data } = await apiClient.put<OpenPlatformConfig>('/open-platform-config', patch); setConfig(data) }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded shadow p-6 space-y-4">
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            { key: 'apiEnabled', label: 'OpenAPI', icon: '🔌', desc: '提供 REST API 接口' },
            { key: 'webhookEnabled', label: 'Webhook', icon: '🪝', desc: '事件主动推送' },
            { key: 'sdkEnabled', label: 'SDK 下载', icon: '📦', desc: '多语言客户端 SDK' },
            ...
            <div key={key} className="flex items-center justify-between p-4 border rounded">
              ...
              <Toggle checked={config[key as keyof OpenPlatformConfig] as boolean} onChange={(v) => update({ [key]: v })} />
            </div>
          ))}
        </div>
      </div>
      <div className="bg-white rounded shadow p-6 space-y-4">
        ...
        <div>
          <label className="block text-sm font-medium mb-2">允许调用的 Scope（逗号分隔）</label>
          <input
            value={config.openScopes.join(', ')}
            onChange={(e) => setConfig({ ...config, openScopes: e.target.value.split(',').map((s) => s.trim()).filter(Boolean) })}
            onBlur={() => update({ openScopes: config.openScopes })}
            className="w-full border rounded px-3 py-2 text-sm font-mono"
            placeholder="user:read, role:read, org:read"
          />
          <p className="text-xs text-gray-400 mt-1">定义第三方应用可调用的 API 权限范围</p>
        </div>
      </div>
      <div className="bg-white rounded shadow p-6 space-y-4">
        <h3 className="text-base font-semibold text-gray-700 border-b pb-2">回调地址白名单</h3>
        <div>
          <label className="block text-sm font-medium mb-2">允许的回调域名（逗号分隔）</label>
          <input
            value={whitelist}
            onChange={(e) => setWhitelist(e.target.value)}
            onBlur={() => update({ callbackWhitelist: whitelist.split(',').map((s) => s.trim()).filter(Boolean) })}
            className="w-full border rounded px-3 py-2 text-sm font-mono"
