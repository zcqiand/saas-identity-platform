...
export default function TenantDetail() {
  ...
  const { current, loading, error, fetchTenant, updateTenant, clearError } = useTenantStore()
  ...
  if (error && !current) {
    return (
      <div className="space-y-4">
        <div role="alert" className="text-red-600 bg-red-50 p-3 rounded">{error}</div>
        <button onClick={() => navigate('/platform/tenants')} className="text-blue-600 hover:underline text-sm">
          ← 返回租户列表
        </button>
      </div>
    )
    ...
    <div className="space-y-6 max-w-2xl">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <button onClick={() => navigate('/platform/tenants')} className="text-gray-500 hover:text-gray-700 text-sm">
            ← 返回
          </button>
          <h2 className="text-2xl font-bold">租户配置</h2>
          {current && <span className="text-sm text-gray-400 font-mono">{current.id}</span>}
        </div>
        {saved && <span className="text-green-600 text-sm">保存成功</span>}
      </div>

      {error && (
        <div role="alert" className="text-red-600 bg-red-50 p-2 rounded text-sm">{error}</div>
      )}
      ...
      <section className="bg-white rounded shadow p-6 space-y-4">
        ...
        <div>
          <label htmlFor="td-name" className="block text-sm font-medium mb-1">租户名称</label>
          <input
            id="td-name"
            value={name}
        ...
        <div>
          <label htmlFor="td-logo" className="block text-sm font-medium mb-1">Logo 文本</label>
          <input
            id="td-logo"
            value={logoText}
      ...
      <section className="bg-white rounded shadow p-6 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label htmlFor="td-primary" className="block text-sm font-medium mb-1">主色</label>
          ...
          <div>
            <label htmlFor="td-sidebar" className="block text-sm font-medium mb-1">侧边栏色</label>
            <div className="flex gap-2">
              <input
                type="color"
                value={sidebar}
        ...
        {/* 主题预览 */}
        <div className="flex gap-2">
          <div className="w-8 h-8 rounded border" style={{ background: primary }} title="主色预览" />
          <div className="w-8 h-8 rounded border" style={{ background: sidebar }} title="侧边栏色预览" />
      ...
      {/* 功能与套餐 */}
      <section className="bg-white rounded shadow p-6 space-y-4">
        <h3 className="text-base font-semibold text-gray-700 border-b pb-2">功能与套餐</h3>
        <div>
        ...
        <div>
          <label htmlFor="td-max-users" className="block text-sm font-medium mb-1">最大用户数</label>
          <input
            id="td-max-users"
            type="number"
            min={1}
