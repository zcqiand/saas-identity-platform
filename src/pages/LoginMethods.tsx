...
export default function LoginMethods() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setMethods(m.data); setSsoProviders(s.data); setOauth2Providers(o.data); setLoading(false)
    ...
    <div className="space-y-6">
      ...
      <section className="bg-white rounded shadow p-6 space-y-3">
        ...
        <div className="space-y-2">
          {methods.map((m) => (
            <div key={m.id} className="flex items-center justify-between p-3 border rounded hover:bg-gray-50">
              <div>
                <p className="font-medium text-sm">{m.name}</p>
                <p className="text-xs text-gray-400">{m.description}</p>
              </div>
              <label className="relative inline-flex items-center cursor-pointer">
                <input type="checkbox" checked={m.enabled} onChange={(e) => toggleMethod(m.id, e.target.checked)} disabled={saving} className="sr-only peer" />
      ...
      {/* SSO 提供商 */}
      <section className="bg-white rounded shadow p-6 space-y-3">
        <h3 className="text-base font-semibold text-gray-700 border-b pb-2">单点登录（SSO）</h3>
        <div className="space-y-3">
          {ssoProviders.map((p) => (
            <div key={p.id} className="flex items-center justify-between p-3 border rounded hover:bg-gray-50">
              <div>
                <p className="font-medium text-sm">{p.name} <span className="text-xs text-gray-400 ml-1">({p.type})</span></p>
                <p className="text-xs text-gray-400 font-mono">{p.issuerUrl ?? '未配置'}</p>
              </div>
              <label className="relative inline-flex items-center cursor-pointer">
                <input type="checkbox" checked={p.enabled} onChange={(e) => updateSso(p.id, { enabled: e.target.checked })} disabled={saving} className="sr-only peer" />
                <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-blue-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-600 peer-disabled:opacity-50"></div>
              </label>
            </div>
          ))}
      ...
      {/* OAuth2 提供商 */}
      <section className="bg-white rounded shadow p-6 space-y-3">
        <h3 className="text-base font-semibold text-gray-700 border-b pb-2">OAuth2 登录</h3>
        <div className="space-y-3">
          {oauth2Providers.map((p) => (
            <div key={p.id} className="flex items-center justify-between p-3 border rounded hover:bg-gray-50">
              ...
              <label className="relative inline-flex items-center cursor-pointer">
                <input type="checkbox" checked={p.enabled} onChange={(e) => updateOAuth(p.id, { enabled: e.target.checked })} disabled={saving} className="sr-only peer" />
                <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-blue-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-600 peer-disabled:opacity-50"></div>
              </label>
            </div>
          ))}
