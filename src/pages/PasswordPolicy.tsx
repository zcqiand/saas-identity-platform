...
export default function PasswordPolicyPage() {
  ...
  useEffect(() => {
    apiClient.get<PasswordPolicy>("/password-policy").then((r) => setConfig(r.data));
  }, []);

  const update = async (patch: Partial<PasswordPolicy>) => {
    setSaving(true);
    try {
      const { data } = await apiClient.put<PasswordPolicy>("/password-policy", patch);
      setConfig(data);
    } finally {
      setSaving(false);
    }
  ...
  return (
    <div data-fn="M06.F03.I01" className="space-y-6">
      ...
      <div className="bg-white rounded shadow p-6 space-y-3">
        ...
        <Field label="最小密码长度">
          <input
            ...
            onBlur={() => update({ minLength: config.minLength })}
            onChange={(e) => setConfig({ ...config, minLength: Number(e.target.value) })}
            className="border rounded px-3 py-1.5 w-20 text-center"
          />
        </Field>
        <Field label="必须包含大写字母">
          <label className="relative inline-flex items-center cursor-pointer">
            <input
              ...
              onChange={(e) => update({ requireUppercase: e.target.checked })}
              className="sr-only peer"
            />
            <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-600"></div>
          </label>
        ...
        <Field label="必须包含小写字母">
          <label className="relative inline-flex items-center cursor-pointer">
            <input
              ...
              onChange={(e) => update({ requireLowercase: e.target.checked })}
              className="sr-only peer"
            />
            <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-600"></div>
          </label>
        ...
        <Field label="必须包含数字">
          <label className="relative inline-flex items-center cursor-pointer">
            <input
              ...
              onChange={(e) => update({ requireDigit: e.target.checked })}
              className="sr-only peer"
            />
            <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-600"></div>
          </label>
        ...
        <Field label="必须包含特殊字符">
          <label className="relative inline-flex items-center cursor-pointer">
            <input
              data-fn="M06.F03.I07"
              type="checkbox"
              checked={config.requireSpecial}
              onChange={(e) => update({ requireSpecial: e.target.checked })}
              className="sr-only peer"
            />
            <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-600"></div>
          </label>
        ...
        <Field label="密码过期天数" desc="0 表示永不过期">
          <input
            ...
            onBlur={() => update({ expireDays: config.expireDays })}
            onChange={(e) => setConfig({ ...config, expireDays: Number(e.target.value) })}
        ...
        <Field label="历史密码数量" desc="不可重复使用的最近密码个数">
          <input
            ...
            onBlur={() => update({ historyCount: config.historyCount })}
            onChange={(e) =>
              setConfig({ ...config, historyCount: Number(e.target.value) })
            }
            className="border rounded px-3 py-1.5 w-20 text-center"
          />
        </Field>
