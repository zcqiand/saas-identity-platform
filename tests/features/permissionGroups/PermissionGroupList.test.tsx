...
const FIDS = ["M03.F02.I01","M03.F02.I02","M03.F02.I03","M03.F02.I04","M03.F02.I05"] as const
...
describe('PermissionGroupList', () => {
  fnTest([...FIDS], 'mount 后拉取并渲染权限组列表', async () => {
  ...
  fnTest([...FIDS], '渲染权限组描述', async () => {
    render(<PermissionGroupList />)
    await waitFor(() => expect(screen.getByText('超级管理员')).toBeInTheDocument())
    expect(screen.getByText('拥有所有权限')).toBeInTheDocument()
  })

  fnTest([...FIDS], '渲染权限数', async () => {
    render(<PermissionGroupList />)
    await waitFor(() => expect(screen.getByText('超级管理员')).toBeInTheDocument())
  ...
  fnTest([...FIDS], '编辑权限组流程: 点击编辑 → 表单回填 → 提交', async () => {
    const user = userEvent.setup()
    render(<PermissionGroupList />)
    await waitFor(() => expect(screen.getByText('超级管理员')).toBeInTheDocument())
    const row = screen.getByText('超级管理员').closest('tr')!
    await user.click(within(row).getByRole('button', { name: '编辑' }))
    const heading = await screen.findByText('编辑权限组', { selector: 'h3' })
    expect(heading).toBeInTheDocument()
  })

  fnTest([...FIDS], '删除权限组流程: 点击删除 → 确认 → 列表移除', async () => {
    const user = userEvent.setup()
    render(<PermissionGroupList />)
    await waitFor(() => expect(screen.getByText('超级管理员')).toBeInTheDocument())
    const row = screen.getByText('超级管理员').closest('tr')!
