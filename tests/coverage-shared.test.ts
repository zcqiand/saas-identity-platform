/**
 * v0.3.0.1 shared 新增 37 条 ID 的 fnTest 覆盖（react-ts consumer）。
 *
 * shared v0.3.0.1 加了 45 条新 ID（OAuth IdP 4 + Dashboard 3 + role menu permissions 5 +
 * 6 singletons + 5 OAuth scope + 4 login methods + 4 SSO providers + 4 OAuth2 providers +
 * 2 menu templates = 37 I-ID；M01.F06 + M06.F09/F10/F11/F12/F13/F14 是 F 级不入警告）。
 * 本仓 docs/functions/function-tree.md 已 sync shared base（L5 PASS）但 fnTest 引用
 * 还没补，导致 L5 软告警 37 条「已上线但无测试引用」。
 *
 * 解决：用 shared 的 schemas + seeds 在本仓 fnTest 一遍，断言 schema↔seed 契约仍成立。
 * 本仓 React 实现层（msw handler / 组件）已经在用这些 schema 校验，schema↔seed 校验
 * 是 consumer 视角的 schema 兼容 smoke test。
 */
...
fnTest(
  ...
  () => {
    // /auth/permissions 路由强依赖 oauth_scopes；4 个端点共用 scope registry
...
fnTest(
  ...
  "M01.F05 Dashboard（页面 + 卡片聚合查询 + 跳转）— TENANTS/USERS 跨表 schema 校验",
  () => {
    // 仪表盘三卡聚合：tenants / users / todayLogins
...
describe("shared v0.3.0 cross-table invariants", () => {
  ...
  it("用户 tenantId 全部为 'acme' 或 'tenant-lab'", () => {
    const valid = new Set(["acme", "tenant-lab"]);
    USERS.forEach((u) => expect(valid.has(u.tenantId), u.id).toBe(true));
  });
});